...
from dril.a2c_ppo_acktr.algo.dril import DRIL, PLOT_ENSEMBLE_SIGNAL
...
def main():
    ...
    if args.behavior_cloning or args.dril or args.warm_start:
        ...
        bc_file_name = f'bc_{args.env_name}_d={args.distribution}_policy_ntrajs={args.num_trajs}_seed={args.seed}'
        ...
        if BC_TEST_ON:
            ...
            test_reward = 1
            print(f'\n\n*** Behavior cloning override: {test_reward} ***\n\n')

        #untrained_params = copy.deepcopy(actor_critic.state_dict())
        ...
        #print(f'*** UNTRAINING BC ***')
        #actor_critic.load_state_dict(untrained_params)
...
# If behavior cloning terminate the script early
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            ...
            with torch.no_grad():
                value, action, action_log_prob, recurrent_hidden_states = actor_critic.act(
                    ...
                    rollouts.masks[step], deterministic=DET)
            ...
            for i, info in enumerate(infos):
                if 'episode' in info.keys():
                    ...
                    ma_rewards.append(info['episode']['r'])
                    print(f"Track score = {info['episode']['r']}")
        ...
        if PLOT_ENSEMBLE_SIGNAL:
            fname = f'{args.env_name}_n={args.ensemble_size}_hu={args.ensemble_hidden_size}_ntraj={args.num_trajs}_seed={args.seed}_d={args.distribution}_signals.pt'
            ...
            data = [rl, ensemble_variance_hist, raw_reward_hist, episode_rewards]
            torch.save(data, fpath)
            print(f'Saving rollout to {fpath}')

            print('*'*30)
            print('*' * 30)
            input(f'Need to stop here >>> Press any key to continue and overwrite results')
            print('*' * 30)
            print('*' * 30)
        ...
        """
            ...
            print(f'Saving model to {os.path.join(save_path, f"{model_file_name}_train_nup={j}.pt")}')
        """
        ...
        if (j % args.eval_interval == 0) or np.mean(episode_rewards)>=MAX_REWARD or j == num_updates - 1:
            ...
            file_name = f'{algo}_{args.env_name}_d={args.distribution}_BN3_ntrajs={args.num_trajs}_seed={args.seed}_'\
                        f'ppoepoch={args.ppo_epoch}_lr={args.lr}_steps={args.num_steps}_det={DET}_nup={j}_scr={np.mean(episode_rewards)}'
            ...
            print(f'Saved model to {os.path.join(save_path, f"{file_name}.pt")}')
