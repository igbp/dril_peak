...
class DRIL:
    ...
    def policy_variance(self, q=0.98, envs=None):
        ...
        if PLOT_ENSEMBLE_SIGNAL:
            args = get_args()
            print(f'args: {args.env_name}')
            dir_path_ens = '/home/giovani/hacer/dril/dril/trained_models/ensemble/'
            fname = f'{args.env_name}_n={args.ensemble_size}_hu={args.ensemble_hidden_size}_ntraj={args.num_trajs}_seed={args.seed}_d={args.distribution}_actions.pt'
            fpath = os.path.join(dir_path_ens, fname)

            variance_keys = variance.keys()
            var = {}
            for key in variance_keys:
                var[key] = copy.deepcopy(variance[key])

            qts = {}
            quant_keys = quantiles.keys()
            for key in quant_keys:
                qts[key] = copy.deepcopy(quantiles[key])
                print(f'key: {key}, qts[key]: {qts[key]}')

            dset = {'variance': var, 'quantiles': qts}
            torch.save(dset, fpath)
            print(f"Saving variance and quantiles to {fpath}")
    ...
    def predict_reward(self, actions, states, envs):
        ...
        if PLOT_ENSEMBLE_SIGNAL:
            return torch.FloatTensor(np.array(rewards)[np.newaxis].T), ensemble_variance
        else:
